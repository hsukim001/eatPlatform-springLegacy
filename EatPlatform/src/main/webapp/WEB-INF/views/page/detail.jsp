...
<body>
	...
	<script src="<%=request.getContextPath()%>/resources/js/page/review.js"></script>
			...
			$.getJSON(
				...
				function(data) {				
					...
					$(data.list).each(function() {
					    ...
					    // 리뷰 정보 (유저 ID, 별점, 내용, 태그 등)
					    reviewItem += '<div class="review_info">'
					        ...
					        + '<span>' + this.reviewLike + ' 추천</span>'
					        + '</div>'; // review_info div 끝

					    // 버튼들 (수정, 삭제, 추천, 신고)
					    reviewItem += '<div class="review_buttons">'
					    ...
					    // 리뷰 이미지들
					    reviewItem += '<div class="review_images">';
					    $(this.reviewImageList).each(function() {
					        ...
					        reviewItem += '<div class="review-image">'
					            + '<a href="' + ReviewImgUrl + '" target="_blank">'
					            + '<img width="100px" height="100px" src="' + ReviewImgUrl + '" />'
					            + '</a>'
					            + '</div>';
					    });
					    reviewItem += '</div>'; // review_images div 끝

					    // 리뷰 댓글 입력란
					    reviewItem += '<div class="review_replies" id="review_' + this.reviewId + '_replies"></div>'
