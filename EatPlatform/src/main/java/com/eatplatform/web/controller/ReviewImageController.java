...
@Controller
@RequestMapping(value = "/page/image")
@Log4j
public class ReviewImageController {
	...
	// 첨부 파일 업로드 처리(POST)
	@PostMapping("/attach")
	public String attachPOST(ReviewImageVO reviewImageVO) {
		log.info("attachPost()");
		log.info("attachDTO = " + reviewImageVO);
	...
	public void list(Model model) {
		// 첨부 파일 id 리스트를 Model에 추가하여 전달
		model.addAttribute("idList", reviewImageService.getAllImageId());
		log.info("list()");
	}
    ...
    public ResponseEntity<Resource> download(int reviewImageId) throws IOException {
        ...
        headers.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" 
              + reviewImageName + "." + reviewImageExtension);
